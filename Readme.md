...
- **Python 3.x** (Note: The script does not work with Python versions 3.14 or higher due to compatibility issues with OpenCV-Python in these versions. OpenCV-Python is only available for Python 3.13 or earlier.)
...
- **Windows**:  
On Windows, the script should work after running the above command in your command prompt or terminal. You may need to install some additional dependencies for OpenCV, such as `ffmpeg` or the `opencv-contrib-python` package, depending on your system configuration.

- **Linux**:  
On Linux, the dependencies should also be installed with the same `pip install -r requirements.txt` command. However, you might need to install OpenCV libraries separately in some cases:
...
4. **Adjust Video Comparison Logic**  
